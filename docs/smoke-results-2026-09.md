...
| E11 | `profileViewBeacon` | `/identity/dash/profileViews` | **nothing** — see the note below | Not reachable from any action. To exercise it at all: service worker console → `import('./src/background/voyager.js').then(v => v.viewProfile('<publicId>', { beacon: true }))` | `{ status: 'sent', sentAt }`. **This endpoint is currently unreachable in shipped code**: `viewProfile(publicId, { beacon })` has exactly one caller (`outreach.js:46`) and it never passes `beacon`, and `beacon` is not a param on `outreach.view` in the contract. The plain visit relies on the `profileView` GET, which is what LinkedIn actually records, so nothing is broken by this — but the beacon path is untested and unused. Verify or delete | | |
| E12 | `reactions` / `comments` urn rewrite | `urn:li:ugcPost:<id>` rewritten to `urn:li:activity:<id>` | `post.engagers` | `lit engagers "<a document or article share permalink>" --kind both --json` | `{ engagers: [...] }` non-empty. Right for most posts; document and article shares are the case worth checking | | |
