...
| `network.unfollowCount` | `{}` | `{ count: number, sample?: string[] }` — popup only; any other origin gets `UNAUTHORIZED`. `count` is read from the following page's own header ("You are following 785 people out of your network") and falls back to the rows currently loaded when that line is absent. `sample` is the first 10 names, when reading them costs nothing |
| `network.unfollowAll` | `{ limit?: number, dryRun?: boolean }` | `{ unfollowed: number, attempted: number, names: string[], stopped?: 'limit'\|'end'\|'error', error?: string }` — popup only; any other origin gets `UNAUTHORIZED`. `limit` is 1–5000 and stops the run after that many *successful* unfollows; omit it to work through the whole list. `dryRun: true` walks the same list by the same selectors and clicks nothing, returning in `names` the people it would have unfollowed (`unfollowed` and `attempted` are then `0`). `attempted` counts every button pressed, so `attempted - unfollowed` is how many did not take. `stopped` says why the run ended: `limit` (your number was reached), `end` (the list ran out) or `error` (the tab navigated away, was closed, or LinkedIn showed a checkpoint — `error` carries the sentence) |
